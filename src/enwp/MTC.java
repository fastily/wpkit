...
public final class MTC
{
	...
	public static void main(String[] args) throws Throwable
	{
		...
		if (l.hasOption('u'))
			...
			procList(new ArrayList<>(Files.readAllLines(Paths.get(l.getOptionValue('f')), Charset.defaultCharset())));
		else
	 ...
	 */
	private static void doTransfer(String title)
	{
		...
		try
		{
			...
			text = cleanupText(rawhtml.substring(rawhtml.indexOf("{{Info"), rawhtml.indexOf("</textarea>")));
		}
		catch (Throwable e)
		{
			e.printStackTrace();
		...
		if (text != null && WTask.downloadFile(title, localFN, enwp))
			com.upload(Paths.get(localFN), title, text, "Transferred from enwp");
	...
	{
		String t = text.replaceAll("(?si)\\{\\{(Green|Red|Yesno|Center|Own|Section link|Trademark|PD\\-logo)\\}\\}\n?", "");
		t = t.replaceAll("(?si)\\{\\{(\\QCc-by-sa-3.0-migrated\\E).*?\\}\\}\n?", "");
